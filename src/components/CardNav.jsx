...
const CardNav = ({
  ...
  const closeMenu = () => {
    ...
    // Force close any running animations
    if (tlRef.current) {
      tlRef.current.kill();
      const newTl = createTimeline();
      if (newTl) {
        ...
        <div
          ...
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              ...
              ref={setCardRef(idx)}
              style={{
                background: `linear-gradient(145deg, ${item.bgColor} 0%, ${item.bgColor} 90%, ${item.bgColor} 100%)`,
                color: item.textColor,
                boxShadow: '0 2px 8px -1px rgba(0, 0, 0, 0.1), 0 1px 4px -1px rgba(0, 0, 0, 0.05)',
                position: 'relative',
                overflow: 'hidden'
              }}>
              <div className="absolute inset-0 bg-gradient-to-b from-white/3 via-white/8 to-white/3"></div>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[33%] bg-white/10 rounded-full blur-md"></div>
              <div
                className="nav-card-label font-normal font-['Montserrat'] tracking-[-0.5px] text-[18px] md:text-[22px] relative z-10 text-shadow-sm">
                {item.label}
              </div>
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  <Link
                    key={`${lnk.label}-${i}`}
                    to={lnk.to || '#'}
                    className="nav-card-link inline-flex items-center gap-[6px] no-underline cursor-pointer transition-all duration-300 hover:opacity-80 text-[15px] md:text-[16px] font-['Montserrat'] relative z-10 text-shadow-sm"
