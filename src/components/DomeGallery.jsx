...
export default function DomeGallery({
  ...
  const applyTransform = (xDeg, yDeg) => {
    ...
    if (el) {
      ...
      el.style.transform = `translateZ(calc(var(--radius) * -1)) rotateX(${xDeg}deg) rotateY(${yDeg}deg) scale(${scale})`;
    }
  };
  ...
  const bind = useGesture({
    onDrag: ({ offset: [x, y], first, last, event, ...state }) => {
      if (first) {
        stopInertia();
        startPosRef.current = { x, y };
      }

      if (state.direction[0] !== 0 || state.direction[1] !== 0) {
        state.event.preventDefault();
      }

      const dx = (x - (startPosRef.current?.x || 0)) * 0.5;
      const dy = (y - (startPosRef.current?.y || 0)) * 0.5;
      ...
      if (last) {
        ...
        startInertia(state.velocity[0], state.velocity[1]);
      }
    },
    ...
    onWheel: ({ event, ...state }) => {
      event.preventDefault();
      const delta = -Math.sign(state.delta[1]) * 2;
      const nextY = rotationRef.current.y + delta;
  ...
  const calculateRadius = useCallback(() => {
    if (typeof window === 'undefined') return minRadius;
    const isMobile = window.innerWidth < 768; // md breakpoint
    return isMobile ? Math.min(500, minRadius) : minRadius;
  }, [minRadius]);
